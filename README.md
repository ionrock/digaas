...
- `duration`: the time, in seconds, it took (since `start_time`) to
...
`start_time` from a timestamp taken locally on a digaas node. It is important
to sync the servers to network time for this reason.
...
```
curl -H 'Content-type: application/json' -H 'Accept: application/json' ${API}/observers/6
```
...
```
curl -H 'Content-type: application/json' -H 'Accept: application/json' ${API}/stats/3/summary
```
...
- `observers_by_type`: statistics on observer `duration`s, broken down by
  observer type
- `observers_by_nameserver`: statistics on observer `duration`s, broken down by
  nameserver.
...
```
curl ${API}/stats/1/plots/propagation_by_type -o propagation_by_type.png
curl ${API}/stats/1/plots/propagation_by_nameserver -o propagation_by_nameserver.png
curl ${API}/stats/1/plots/query -o query.png
```
